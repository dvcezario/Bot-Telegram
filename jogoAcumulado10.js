...
async function validatePhoneNumberAcumulado10(ctx) {
...
bot.action('confirmar_Numeros', async (ctx) => {
    ...
    if (ctx.session.selectedNumbers.length === 10) {
        try {
            await salvarNumerosSelecionadosAcumulado10(ctx.session.selectedNumbers, ctx);
...
async function inserirIDPagamentoNaPlanilha(idUnico) {
    const fileName = 'NumerosSelecionadosAcumulado10.xlsx';
    ...
    try {
        ...
        if (fs.existsSync(fileName)) {
            ...
            const worksheet = workbook.Sheets['NumerosSelecionadosAcumulado10'];
...
function salvarNumerosSelecionadosAcumulado10(selectedNumbers, ctx) {
    const fileName = 'NumerosSelecionadosAcumulado10.xlsx';
    ...
    if (fs.existsSync(filePath)) {
        ...
        xlsx.utils.book_append_sheet(workbook, worksheet, 'NumerosSelecionadosAcumulado10');
        xlsx.writeFile(workbook, filePath);
    }
}
...
module.exports = {
    ...
    salvarNumerosSelecionadosAcumulado10,
