...
// Variável para armazenar o message_id da mensagem "Escolha 15 números"
...
async function validatePhoneNumberTiroCerto(ctx) {
    ...
    bot.on('text', async (ctx) => { // Torna a função de callback assíncrona
        ...
        if (isValidPhoneNumber(response) && ctx.session.awaitingPhoneNumber) {
            ...
            let message = await ctx.reply('Escolha seus 15 números:', Markup.inlineKeyboard(keyboard));
            let messageId = message.message_id;
            await ctx.session.mensagensIDS.push(messageId);

            // Armazena o message_id da mensagem "Escolha 15 números"
...
// Função para excluir o teclado numérico junto com a mensagem "Escolha 15 números"
...
bot.action('voltar', async (ctx) => {
    ...
    // Deleta apenas o teclado numérico junto com a mensagem "Escolha 15 números"
...
bot.action(/^[1-9]\d*$/, async (ctx) => {
    ...
    if (index === -1) {
        // Verifica se o usuário já selecionou 15 números
        if (ctx.session.selectedNumbers.length > 15) {
            // Se o usuário já selecionou 15 números, envie uma mensagem e não permita que ele selecione mais
            const salvarID = await ctx.reply('Por favor, selecione EXATAMENTE 15 números, se for o caso substitua algum número não desejado, por outro desejado...');
    ...
    // Restante do código...
    const salvarId5 = await ctx.editMessageText('Escolha os seus 15 números:', Markup.inlineKeyboard(keyboard));
...
async function handleConfirmAction(ctx) {
    ...
    if (ctx.session.selectedNumbers.length < 15) {
    const salvarID =  await ctx.reply('Por favor, selecione exatamente 15 números antes de confirmar.');
    ctx.session.mensagensIDS.push(salvarID.message_id);
    }
    ...
    if (ctx.session.selectedNumbers.length === 15) {
        ...
        const salvarId = await ctx.reply('Por favor selecione EXATAMENTE 15 números:', Markup.inlineKeyboard(keyboard));
        if (salvarId && salvarId.message_id) {
            ctx.session.mensagensIDS.push(salvarId.message_id);
        }
...
bot.action('confirmar_Numeros', async (ctx) => {
    ...
    if (ctx.session.selectedNumbers.length === 15) {
        try {
            await salvarNumerosSelecionadosTiroCerto(ctx.session.selectedNumbers, ctx);
        ...
        ctx.reply('Por favor, selecione exatamente 15 números antes de confirmar.');
    }
});
...
async function inserirIDPagamentoNaPlanilha(idUnico) {
    const fileName = 'NumerosSelecionadosTiroCerto.xlsx';

    // Adicionando bloqueio ao acessar a planilha
    let release;
    try {
        // Tenta adquirir o bloqueio no arquivo
        release = await lockfile.lock(fileName);

        if (fs.existsSync(fileName)) {
            const workbook = xlsx.readFile(fileName);
            const worksheet = workbook.Sheets['NumerosSelecionadosTiroCerto'];
...
bot.action('alterar_Numeros', async (ctx) => {
    ...
    const salvarId = await ctx.editMessageText('Escolha 15 números:', Markup.inlineKeyboard(keyboard));
    if (salvarId) {
        ctx.session.mensagensIDS.push(salvarId.message_id);
    }
});
...
function salvarNumerosSelecionadosTiroCerto(selectedNumbers, ctx) {
    const fileName = 'NumerosSelecionadosTiroCerto.xlsx';
    const filePath = path.join(__dirname, fileName); // Caminho absoluto do arquivo
    let nomeUsuario = '';
    let id = '';
    ...
    if (fs.existsSync(filePath)) {
        ...
        xlsx.utils.book_append_sheet(workbook, worksheet, 'NumerosSelecionadosTiroCerto');
        xlsx.writeFile(workbook, filePath);
    }
}
...
module.exports = {
    ...
    salvarNumerosSelecionadosTiroCerto,
